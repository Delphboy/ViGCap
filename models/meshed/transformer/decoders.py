...
from models.meshed.containers import Module, ModuleList
from models.meshed.transformer.attention import MultiHeadAttention
from models.meshed.transformer.utils import (
...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        ...
        enc_att1 = (
            self.enc_att(self_att, enc_output[:, 0], enc_output[:, 0], mask_enc_att)
            * mask_pad
        )
        enc_att2 = (
            self.enc_att(self_att, enc_output[:, 1], enc_output[:, 1], mask_enc_att)
            * mask_pad
        )
        enc_att3 = (
...
class MeshedDecoder(Module):
    def __init__(
        ...
        self.register_state(
            "running_mask_self_attention", torch.zeros((1, 1, 0)).byte()
        )
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        try:
            ...
            print("input")
            print(input.shape)
            # print the min and max of input
